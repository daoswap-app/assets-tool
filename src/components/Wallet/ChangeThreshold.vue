...
// 表单数据
const defalutFormValue = { confirmNumber: 1 };
...
const submitForm = async () => {
  ...
  if (checkRet) {
    ...
    const dataEncode = await contract.methods
      .changeThreshold(walletForm.value.confirmNumber)
      .encodeABI();
    const params = {
...
watch(
  ...
  val => {
    loading.value = true;
    if (val) {
      ownerCount.value = parseInt(val.ownerCount);
...
<template>
  <el-dialog
    v-model="formVisible"
    :title="transformI18n('wallet.changeThreshold')"
    width="90%"
    draggable
